...
def main():
    """
    保留i2t_acc值最高的权重epoch_best.pt和最新的权重epoch_latest.pt
    params:
        mac_i2t_acc: 最大的i2t_acc值
        i2t_acc: 最新的i2t_acc值
    """
    max_i2t_acc = 0
    i2t_acc = 0
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.val_data is not None and args.valid_epoch_interval is not None and ((epoch + 1) % args.valid_epoch_interval) == 0:
            ...
            if not args.use_flash_attention:
                i2t_acc = evaluate(model, data, epoch, args, steps)
            else:
        ...
        if args.should_save and num_steps_this_epoch > 0:
            if (epoch + 1) == args.max_epochs or (
                ...
                if i2t_acc > max_i2t_acc:
                    t1 = time.time()
                    save_path = os.path.join(args.checkpoint_path, f"epoch_best.pt")
                    torch.save(
                        {
                            "epoch": epoch + 1,
                            "step": steps,
