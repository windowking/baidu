...
def evaluate(model, data, epoch, args, steps):
    """
    评估模型训练过程中在验证集上的性能(修改部分为加入top5_acc的计算和输出)
    """
    ...
    cumulative_i2t_acc_top5 = torch.zeros([]).cuda(args.local_device_rank, non_blocking=True)
    cumulative_t2i_acc_top5 = torch.zeros([]).cuda(args.local_device_rank, non_blocking=True)
    ...
    with torch.no_grad():
        for i in range(dataloader.num_batches):
            ...
            cumulative_i2t_acc_top5 += ((logits_per_image.topk(k=5, dim=-1)[1] == ground_truth.unsqueeze(1)).any(dim=1).sum()).float()
            cumulative_t2i_acc_top5 += (logits_per_text.topk(k=5, dim=-1)[1] == ground_truth.unsqueeze(1)).any(dim=1).sum().float()
        ...
        dist.all_reduce(cumulative_i2t_acc_top5, op=dist.ReduceOp.SUM)
        dist.all_reduce(cumulative_t2i_acc_top5, op=dist.ReduceOp.SUM)

        loss = cumulative_loss / num_elements
        #求top1的准确率
        i2t_acc_top1 = cumulative_i2t_acc / num_elements
        t2i_acc_top1 = cumulative_t2i_acc / num_elements
        #求top5的准确率
        i2t_acc_top5 = cumulative_i2t_acc_top5 / num_elements
        t2i_acc_top5 = cumulative_t2i_acc_top5 / num_elements
        ...
        logging.info(
            ...
            f"Image2Text Acc_top1: {i2t_acc_top1.item() * 100:.2f} | " 
            f"Text2Image Acc_top1: {t2i_acc_top1.item() * 100:.2f} | " 
            f"Image2Text Acc_top5: {i2t_acc_top5.item() * 100:.2f} | " 
            f"Text2Image Acc_top5: {t2i_acc_top5.item() * 100:.2f} | " 
    ...
    """
    此处返回图片到文本的top1准确率，是用于main.py中保存i2t_acc_top1最高的权重
    见main.py文件273和277行使用
    """
    return i2t_acc_top1.item() * 100
